	...
	protected static void printBoard(int[] gameState) {
		...
		for (int row = 1; row <= topHalfRows; row++) {
			for (int spike = 13; spike <= 24; spike++) {
				...
				if (Math.abs(gameState[spike]) < row) {
					System.out.print(" _ ");
				} else {
					if (gameState[spike] < 0) {
						System.out.print(" R ");
					} else if (gameState[spike] > 0) {
						System.out.print(" W ");
					}
				}
			}
		...
		System.out.println("************************************ Red Captured: "+ Math.abs(gameState[0]));
		for (int row = 1; row <= bottomHalfRows; row++) {
			for (int spike = 12; spike >= 1; spike--) {
				if (spike == 6){
					System.out.print("║");
				}
				if (Math.abs(gameState[spike]) <= (bottomHalfRows - row)) {
					System.out.print(" _ ");
				} else {
					if (gameState[spike] < 0) {
						System.out.print(" R ");
		...
		System.out.println("************************************ White Captured: " + Math.abs(gameState[25]));
